...
class MoELoRAModel(torch.nn.Module):
    ...
    def __init__(self, config, model):
        ...
        mark_lora_layernorm_cls_trainable(self.model, self.peft_config.task_type, self.peft_config.bias)
        ...
        for key in key_list:
            ...
            if target_module_found:
                ...
                if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
                    ...
                    new_module = MoELayer(target.in_features, target.out_features, **kwargs)
...
def mark_lora_layernorm_cls_trainable(model: nn.Module, task_type, bias: str = "none") -> None:
...
class MoELayer(nn.Module):
    def __init__(self, in_features, out_features, num_experts, lora_alpha=1, lora_dropout=0.5, fan_in_fan_out=False, merge_weights=True):
        super(MoELayer, self).__init__()
        self.num_experts = num_experts
        self.merge = merge_weights
        self.dropout_rate = lora_dropout
        self.lora_alpha = lora_alpha
        self.fan_in_fan_out = fan_in_fan_out
        self.experts = nn.ModuleList([LoRALinear(in_features, out_features, self.merge, 2**i, self.lora_alpha, self.dropout_rate) for i in range(num_experts)])
    ...
    def forward(self, x):
        ...
        # output = torch.bmm(gate_score, expert_outputs).squeeze(1)
        return output
...
if is_bnb_available():
    ...
    class Linear8bitLt(bnb.nn.Linear8bitLt, LoRALinear):
        # Lora implemented in a dense layer
        def __init__(
            self,
            in_features,
    ...
    class MergedLinear8bitLt(bnb.nn.Linear8bitLt, LoRALinear):
        # Lora implemented in a dense layer
        def __init__(
            self,
            in_features: int,
